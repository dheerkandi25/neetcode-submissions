...
public:
    ...
    int dfs(int i,int total,vector<int>stones,int stoneSum,int target) {
        ...
        return dp[i][total]=min(dfs(i+1,total,stones,stoneSum,target),dfs(i+1,total+stones[i],stones,stoneSum,target));
    }
};